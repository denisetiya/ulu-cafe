<x-layout>
    <div class="min-h-[80vh] flex items-center justify-center container mx-auto px-6 py-20">
        <div class="bg-[var(--color-dark-card)] p-8 rounded-2xl w-full max-w-md shadow-2xl border border-white/5">
            <h2 class="text-3xl font-bold mb-8 text-center">Masuk</h2>

            @if(session('error'))
                <div class="bg-red-500/20 border border-red-500 text-red-500 px-4 py-3 rounded mb-6 text-sm">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)]">
                    @error('email')
            ...
            <p class="text-center text-gray-400 mt-8 text-sm">
                Belum punya akun? <a href="{{ route('register') }}" class="text-[var(--color-primary)] hover:underline">Daftar sekarang</a>
            </p>
        </div>
    </div>
</x-layout>
