<x-dashboard-layout title="Edit Menu">
    <div class="flex flex-col justify-center items-center p-6">
        <div class="bg-[var(--color-dark-card)] w-full max-w-2xl rounded-2xl shadow-2xl border border-white/10 flex flex-col">
            <div class="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                <h2 class="text-xl font-bold text-white">Edit Menu</h2>
            ...
            <div class="p-6">
                <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ imagePreview: '{{ $product->image ? (Str::startsWith($product->image, 'http') ? $product->image : asset('storage/' . $product->image)) : '' }}' }">
                    @csrf @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ $product->name }}" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)] placeholder-gray-600 transition" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-2">Harga (Rp)</label>
                            <input type="number" name="price" value="{{ $product->price }}" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)] placeholder-gray-600 transition" required>
                        </div>
                    </div>
                    ...
                    <div class="bg-[var(--color-dark-bg)]/50 p-4 rounded-xl border border-white/5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="discount_amount" value="{{ $product->discount_amount }}" step="0.01" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-[var(--color-primary)] transition">
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    <select name="discount_type" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-[var(--color-primary)] appearance-none transition">
                                        <option value="fixed" {{ $product->discount_type == 'fixed' ? 'selected' : '' }}>Nominal (Rp)</option>
                                        <option value="percent" {{ $product->discount_type == 'percent' ? 'selected' : '' }}>Persen (%)</option>
                                    </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)] placeholder-gray-600 transition resize-none">{{ $product->description }}</textarea>
                    </div>
                </form>
            </div>
