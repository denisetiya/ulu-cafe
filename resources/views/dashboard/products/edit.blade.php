        ...
        <div class="bg-[var(--color-dark-card)] p-6 rounded-xl">
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $product->name }}" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)]" required>
                </div>
                <div>
                    <label class="block text-sm text-gray-400 mb-2">Harga</label>
                    <input type="number" name="price" value="{{ $product->price }}" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)]" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full bg-[var(--color-dark-input)] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--color-primary)]">{{ $product->description }}</textarea>
                </div>
                <div>
                    <label class="block text-sm text-gray-400 mb-2">Gambar Menu</label>
                    @if($product->image)
                        <div class="mb-2">
                            <img src="{{ Str::startsWith($product->image, 'http') ? $product->image : asset('storage/' . $product->image) }}" alt="Current Image" class="w-32 h-32 object-cover rounded-lg">
                        </div>
                    @endif
                ...
                <button type="submit" class="bg-[var(--color-primary)] text-white px-6 py-3 rounded-lg font-bold hover:bg-opacity-90 transition">Update Menu</button>
            </form>
        </div>
    </div>
</x-dashboard-layout>
